...
func (h *Hub) GetActivity(c echo.Context) error {
	...
	if h.etherfaceClient != nil && activity != nil && activity.Type == filter.TypeUnknown && activity.Calldata != nil {
...
func (h *Hub) GetAccountActivities(c echo.Context) (err error) {
	...
	for _, activity := range activities {
		if h.etherfaceClient != nil && activity.Type == filter.TypeUnknown && activity.Calldata != nil {
			activity.Calldata.ParsedFunction, _ = h.etherfaceClient.Lookup(c.Request().Context(), activity.Calldata.FunctionHash)
		}
	}
