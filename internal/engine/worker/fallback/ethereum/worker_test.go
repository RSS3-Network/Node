		...
		},
		{
			...
			arguments: arguments{
				task: &source.Task{
					...
					ChainID: 2331,
					Header: &ethereum.Header{
						Hash:         common.HexToHash("0xeb5d973e04135894136247dafeacafebd24c652604d10fb1a52af638f701cbf7"),
						ParentHash:   common.HexToHash("0x4f564e794fa12916d35e1ade4510a22dca293fabec7c63d214353ecaa94a059e"),
						UncleHash:    common.HexToHash("0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347"),
						Coinbase:     common.HexToAddress("0x4200000000000000000000000000000000000011"),
						Number:       lo.Must(new(big.Int).SetString("206841", 0)),
						GasLimit:     30000000,
						GasUsed:      71513,
						Timestamp:    1707298002,
						BaseFee:      lo.Must(new(big.Int).SetString("50", 0)),
						Transactions: nil,
					},
					Transaction: &ethereum.Transaction{
						BlockHash: common.HexToHash("0xd138fed2385c2e924155194e7300a160c5d506fa7d02c3093b0158ac33bda061"),
						From:      common.HexToAddress("0x3B6D02A24Df681FFdf621D35D70ABa7adaAc07c1"),
						Gas:       21000,
						GasPrice:  lo.Must(new(big.Int).SetString("1500000050", 10)),
						Hash:      common.HexToHash("0xd138fed2385c2e924155194e7300a160c5d506fa7d02c3093b0158ac33bda061"),
						Input:     nil,
						To:        lo.ToPtr(common.HexToAddress("0xb461c1521eE9D96a9C950337f0851b79bD66CAe1")),
						Value:     lo.Must(new(big.Int).SetString("200000000000000000000000", 0)),
						Type:      2,
						ChainID:   lo.Must(new(big.Int).SetString("2331", 0)),
					},
					Receipt: &ethereum.Receipt{
						BlockHash:         common.HexToHash("0xeb5d973e04135894136247dafeacafebd24c652604d10fb1a52af638f701cbf7"),
						BlockNumber:       lo.Must(new(big.Int).SetString("206841", 0)),
						ContractAddress:   nil,
						CumulativeGasUsed: 71513,
						EffectiveGasPrice: hexutil.MustDecodeBig("0x59682f32"),
						GasUsed:           21000,
						L1GasPrice:        lo.Must(new(big.Int).SetString("12817993018", 0)),
						L1GasUsed:         lo.Must(new(big.Int).SetString("3988", 0)),
						L1Fee:             lo.Must(new(big.Int).SetString("1022363123115680000", 0)),
						FeeScalar:         lo.Must(new(big.Float).SetString("20000")),
						Logs:              []*ethereum.Log{},
						Status:            1,
						TransactionHash:   common.HexToHash("0xd138fed2385c2e924155194e7300a160c5d506fa7d02c3093b0158ac33bda061"),
						TransactionIndex:  1,
					},
				},
				config: &config.Module{
			...
			want: &schema.Feed{
				ID:      "0xd138fed2385c2e924155194e7300a160c5d506fa7d02c3093b0158ac33bda061",
				Network: filter.NetworkRSS3Testnet,
				Index:   1,
				...
				Type:    filter.TypeTransactionTransfer,
				Fee: &schema.Fee{
					Amount:  lo.Must(decimal.NewFromString("1022394623116730000")),
					Decimal: 18,
				},
				Actions: []*schema.Action{
					{
						...
						Metadata: metadata.TransactionTransfer{
							Name:     "RSS3",
							Symbol:   "RSS3",
							Value:    lo.ToPtr(lo.Must(decimal.NewFromString("200000000000000000000000"))),
							Decimals: 18,
						},
					},
				},
