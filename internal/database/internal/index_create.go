...
func (ic *IndexCreate) check() error {
	if _, ok := ic.mutation.FeedID(); !ok {
		return &ValidationError{Name: "feed_id", err: errors.New(`internal: missing required field "Index.feed_id"`)}
	}
	if _, ok := ic.mutation.Owner(); !ok {
		return &ValidationError{Name: "owner", err: errors.New(`internal: missing required field "Index.owner"`)}
	}
	if v, ok := ic.mutation.Owner(); ok {
		if err := index.OwnerValidator(v); err != nil {
			return &ValidationError{Name: "owner", err: fmt.Errorf(`internal: validator failed for field "Index.owner": %w`, err)}
		}
	}
	if _, ok := ic.mutation.Chain(); !ok {
		return &ValidationError{Name: "chain", err: errors.New(`internal: missing required field "Index.chain"`)}
	}
	if v, ok := ic.mutation.Chain(); ok {
		if err := index.ChainValidator(v); err != nil {
			return &ValidationError{Name: "chain", err: fmt.Errorf(`internal: validator failed for field "Index.chain": %w`, err)}
		}
	}
	if _, ok := ic.mutation.Platform(); !ok {
		return &ValidationError{Name: "platform", err: errors.New(`internal: missing required field "Index.platform"`)}
	}
	if _, ok := ic.mutation.Tag(); !ok {
		return &ValidationError{Name: "tag", err: errors.New(`internal: missing required field "Index.tag"`)}
	}
	if v, ok := ic.mutation.Tag(); ok {
		if err := index.TagValidator(v); err != nil {
			return &ValidationError{Name: "tag", err: fmt.Errorf(`internal: validator failed for field "Index.tag": %w`, err)}
		}
	}
	if _, ok := ic.mutation.GetType(); !ok {
		return &ValidationError{Name: "type", err: errors.New(`internal: missing required field "Index.type"`)}
	}
	if v, ok := ic.mutation.GetType(); ok {
		if err := index.TypeValidator(v); err != nil {
			return &ValidationError{Name: "type", err: fmt.Errorf(`internal: validator failed for field "Index.type": %w`, err)}
		}
	}
	if _, ok := ic.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`internal: missing required field "Index.status"`)}
	}
	if v, ok := ic.mutation.Status(); ok {
		if err := index.StatusValidator(v); err != nil {
			return &ValidationError{Name: "status", err: fmt.Errorf(`internal: validator failed for field "Index.status": %w`, err)}
		}
	}
	if _, ok := ic.mutation.Direction(); !ok {
		return &ValidationError{Name: "direction", err: errors.New(`internal: missing required field "Index.direction"`)}
	}
	if v, ok := ic.mutation.Direction(); ok {
		if err := index.DirectionValidator(v); err != nil {
			return &ValidationError{Name: "direction", err: fmt.Errorf(`internal: validator failed for field "Index.direction": %w`, err)}
		}
	}
	if _, ok := ic.mutation.Index(); !ok {
		return &ValidationError{Name: "index", err: errors.New(`internal: missing required field "Index.index"`)}
	}
	if _, ok := ic.mutation.Timestamp(); !ok {
		return &ValidationError{Name: "timestamp", err: errors.New(`internal: missing required field "Index.timestamp"`)}
	}
	if _, ok := ic.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`internal: missing required field "Index.created_at"`)}
	}
	if _, ok := ic.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`internal: missing required field "Index.updated_at"`)}
	}
	return nil
}
